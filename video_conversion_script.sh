...
then
	...
	then
		...
		then
			ffmpeg -i $FILE -strict experimental -y -acodec aac \
				-ac 2 -ab 160k -s 800x480 -vcodec libx264 \
				-preset ipod640 -preset slow -f mp4 \
				-threads 0 -crf 22 $LOC$CONVERTED$MP4
		else
			ffmpeg -i $FILE -strict experimental -y -acodec aac \
				-ac 2 -ab 160k -vcodec libx264 \
				-preset ipod640 -preset slow -f mp4 \
				-threads 0 -crf 22 $LOC$CONVERTED$MP4
		fi
	fi
	...
	then
		...
		then
			ffmpeg -i $FILE -strict experimental -y -acodec aac \
				-ac 2 -ab 160k -s 480x320 -vcodec libx264 \
				-preset ipod640 -preset slow -f mp4 \
				-threads 0 -crf 22 $LOC$CONVERTED$MP4
		else
			ffmpeg -i $FILE -strict experimental -y -acodec aac \
	...
	then
		...
		then
			ffmpeg -i $FILE -strict experimental -y -acodec aac \
				-ac 2 -ab 160k -s 854x480 -vcodec libx264 \
				-preset ipod640 -preset slow -f mp4 \
				-threads 0 -crf 22 $LOC$CONVERTED$MP4
		else
			ffmpeg -i $FILE -strict experimental -y -acodec aac \
	...
	then
		if [ $WIDTH -gt 480 ] && [ $HEIGHT -gt 320 ]
		then
			ffmpeg -i $FILE -strict experimental -acodec aac \
				-ac 2 -ab 160k -s 480x320 -vcodec libx264 \
				-preset ipod640 -preset slow -b 1200k \
	...
	then
		...
		then
			ffmpeg -i $FILE -strict experimental -acodec aac \
				-ac 2 -ab 160k -s 640x480 -vcodec libx264 \
				-preset ipod640 -preset slow -b 1200k \
				-f mp4 -threads 0 $LOC$CONVERTED$MP4	
		else
			ffmpeg -i $FILE -strict experimental -acodec aac \
	...
	then
		...
		then
			ffmpeg -i $FILE -strict experimental -acodec aac \
				-ac 2 -ab 160k -s 1024x768 -vcodec libx264 \
				-preset ipod640 -preset slow -b 1200k \
				-f mp4 -threads 0 $LOC$CONVERTED$MP4	
		else
			ffmpeg -i $FILE -strict experimental -acodec aac \
	...
	then
		...
		then
			ffmpeg -i $FILE -strict experimental -s 480x270 \
				-b 512000 -ar 24000 -ab 64000 -f psp \
	...
	if [ $FORMAT -eq 7 ]
	then
		ffmpeg -i $FILE -codec:v libtheora -qscale:v 8 \
			-codec:a libvorbis -qscale:a 6 $LOC$CONVERTED$OGV
	fi
	...
	then
		ffmpeg -i $FILE -strict experimental -acodec aac -ac 2 \
			-ab 160k -vcodec libx264 -preset slow -f mp4 \
			-crf 22 $LOC$CONVERTED$MP4
	fi

	# WebM
	if [ $FORMAT -eq 9 ]
	then
		ffmpeg -i $FILE -f webm -vcodec libvpx -acodec libvorbis \
			-ab 160k -crf 22 $LOC$CONVERTED$WEBM
	fi
